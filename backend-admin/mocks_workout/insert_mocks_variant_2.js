...
// Shift coordinates AGAIN (different direction) to create "Variant 2" territories
// Previous was 0.005, 0.005. Let's go negative and larger.
const LAT_OFFSET = -0.008;
const LON_OFFSET = 0.003;
...
const mocks = [
    {
        ...
        sourceName: "Mock Apple Watch - Variant 2",
        metadata: {
            "HKMetadataKeyIndoorWorkout": false
        },
        route: [
            // Base coordinates shifted by NEW offsets
...
async function insertMocks() {
    console.log(`Inserting ${mocks.length} VARIANT 2 mocks into ${MOCK_COLLECTION}...`);
    const batch = db.batch();

    for (const mock of mocks) {
        const ref = db.collection(MOCK_COLLECTION).doc(mock.id);
        batch.set(ref, mock);
        console.log(`- Prepared: ${mock.id} (${mock.type}) [Variant 2]`);
    }

    await batch.commit();
    console.log('Variant 2 mocks inserted successfully!');
}
